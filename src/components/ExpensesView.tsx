...
export default function ExpensesView({
  ...
  const [paymentMode, setPaymentMode] = React.useState<Expense['paymentMode']>('Cash (CSP Limit)');
  const [notes, setNotes] = React.useState('');
  const [deductFromWallet, setDeductFromWallet] = React.useState(false);
  ...
  const filteredExpenseVal = filteredExpenses.reduce((sum, item) => sum + item.amount, 0);
  ...
  // Payment mode math
  const paymentModeSummary = ['Cash (CSP Limit)', 'SSO Wallet', 'UPI/Bank', 'Personal Cash'].reduce((acc, mode) => {
  ...
  const handleAddExpense = (e: React.FormEvent) => {
    ...
    if (isOperatorDisabled) {
      alert("प्रशासक द्वारा ऑपरेटरों के लिए दुकान खर्च रिकॉर्ड करना ब्लॉक किया गया है। (Operator expense logging is disabled by Administrator)");
      return;
    }
    ...
    // Check balance if deduction is checked
    if (deductFromWallet) {
    ...
    // Update balances if deduct requested
    let updatedWallet = { ...wallet };
    let updatedEmitraWallet = { ...emitraWallet };

    if (deductFromWallet) {
      if (paymentMode === 'Cash (CSP Limit)') {
    ...
    // Build log statement
    const systemAction = `Recorded Shop Expense [${category}]: ${description} (${formatINR(parsedAmount)}) via ${paymentMode}. ${
      deductFromWallet ? 'Wallet balance deducted.' : 'Tracked offline.'
    }`;
    ...
    // Reset Form
    setDescription('');
    setCategory('Printing & Ink');
    setAmount('');
    setNotes('');
    setDeductFromWallet(false);
    setIsAdding(false);

    alert(`Successfully recorded expense of ${formatINR(parsedAmount)}.`);
  };

  // Handle Deletion
  const handleDeleteExpense = (expenseId: string) => {
    if (isOperatorDisabled) {
      alert("प्रशासक द्वारा ऑपरेटरों के लिए दुकान खर्च को हटाना ब्लॉक किया गया है। (Operator expense deletion is disabled by Administrator)");
      return;
    }
    const targetExp = expenses.find(e => e.id === expenseId);
    if (!targetExp) return;

    if (!window.confirm(`Are you sure you want to delete the expense entry: "${targetExp.description}"? (Note: Deducted wallet balances will NOT be automatically reverse-refunded; edit wallet balances manually in Admin tab if necessary)`)) {
      return;
    }
      ...
      securityLogs: [
        {
          ...
          operatorName: currentUser?.name || 'Admin',
          role: currentUser?.role || 'Admin',
          action: `Deleted Shop Expense entry: ${targetExp.description} (${formatINR(targetExp.amount)})`,
          status: 'Success',
          ipAddress: '47.11.134.19',
          device: 'Expenses Management',
          browser: 'SmartSPE Portal'
    ...
    onUpdateState(updatedState);
    alert("Expense entry removed successfully.");
  };
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className={`p-5 rounded-3xl border flex items-center justify-between ${
          ...
          <div>
            ...
            <span className="text-[10px] text-slate-400 mt-0.5 block">{expenses.length} unique records</span>
          </div>
        ...
        <div className={`p-5 rounded-3xl border flex items-center justify-between ${
          ...
          <div>
            <span className="text-xs text-slate-400 block font-medium uppercase tracking-wider">Filtered Items Total</span>
            <span className="text-2xl font-black font-mono tracking-tight text-blue-600 dark:text-blue-400 mt-1 block">
              {formatINR(filteredExpenseVal)}
            </span>
            <span className="text-[10px] text-slate-400 mt-0.5 block">{filteredExpenses.length} matches</span>
          </div>
          <div className="p-4 bg-blue-500/15 text-blue-600 rounded-2xl">
            <Filter size={24} />
          </div>
        </div>
        ...
        <div className={`p-5 rounded-3xl border flex items-center justify-between ${
          ...
          <div>
            ...
            <span className="text-[10px] text-slate-400 mt-0.5 block">Used for cash expense debits</span>
          </div>
        ...
        <div className={`p-6 rounded-3xl border animate-fade-in ${
          ...
          <div className="flex items-center justify-between pb-4 border-b border-slate-100 dark:border-slate-800 mb-5">
            <h3 className="font-bold text-sm tracking-tight text-slate-900 dark:text-white flex items-center gap-1.5">
              <span className="p-1 bg-rose-500/15 text-rose-600 rounded-lg"><Receipt size={14} /></span>
              Add New Shop Expense Entry (नया दुकान खर्च जोड़ें)
            </h3>
          ...
          <form onSubmit={handleAddExpense} className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div className="md:col-span-2 space-y-1.5">
              <label className="text-[11px] font-bold text-slate-400 block uppercase tracking-wider">
                Expense Description / Purpose (खर्च का विवरण) *
              </label>
              <input
                type="text"
                required
                placeholder="e.g. WiFi Fiber Broadband monthly recharge"
                value={description}
            ...
            <div className="space-y-1.5">
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  setPaymentMode(val);
                  // Turn off deduct check if not wallet-based
            ...
            <div className="space-y-1.5">
              <label className="text-[11px] font-bold text-slate-400 block uppercase tracking-wider">
                Spent By / Staff member (स्टाफ का नाम) *
              </label>
              <select
                value={staffName}
            ...
            <div className="md:col-span-1 flex items-center justify-start md:pt-6">
              {(paymentMode === 'Cash (CSP Limit)' || paymentMode === 'SSO Wallet') ? (
                <label className="flex items-center gap-2 cursor-pointer select-none">
                  ...
                  <span className="text-xs font-semibold text-slate-600 dark:text-slate-300">
                    Deduct amount directly from selected wallet limit
                  </span>
                </label>
              ) : (
            ...
            <div className="md:col-span-1 flex items-end md:pt-4">
              <button
                type="submit"
                className="w-full py-2.5 bg-rose-600 hover:bg-rose-700 text-white rounded-xl text-xs font-bold cursor-pointer transition-colors shadow-lg shadow-rose-600/10"
              >
                Post Expense Entry
              </button>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className={`p-6 rounded-3xl border ${
          ...
          <h3 className="font-bold text-xs font-display uppercase tracking-wider text-slate-400 mb-4 flex items-center gap-1.5">
            <Trash2 className="rotate-180" size={13} /> Category Wise Spent Ledger
          </h3>
          <div className="space-y-4">
            {categories.map(cat => {
              const spent = categorySummary[cat] || 0;
              const pct = totalExpenseVal > 0 ? (spent / totalExpenseVal) * 100 : 0;
              return (
        ...
        <div className={`p-6 rounded-3xl border ${
          ...
          <div className="space-y-4">
            {['Cash (CSP Limit)', 'SSO Wallet', 'UPI/Bank', 'Personal Cash'].map(mode => {
      ...
      <div className={`p-6 rounded-3xl border ${
        ...
        <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-4 pb-4 border-b border-slate-100 dark:border-slate-800 mb-4">
          <div>
            <h3 className="font-bold text-xs font-display uppercase tracking-wider text-slate-400">
              Expenses Historical Registry
            </h3>
            <p className="text-[10px] text-slate-400">Showing {filteredExpenses.length} filters match elements</p>
          </div>
        ...
        {filteredExpenses.length === 0 ? (
          <div className="text-center py-12 text-slate-400 space-y-2">
            ...
            <p className="text-xs text-slate-400/80">Try resetting filters or registering a new expense on top.</p>
          </div>
        ) : (
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800/50">
                {filteredExpenses.map((exp) => (
                  ...
                  >
                    ...
                    <td className="py-3 px-4 text-right font-mono font-black text-rose-600 dark:text-rose-450">
                      {formatINR(exp.amount)}
                    </td>
                    <td className="py-3 px-4 text-center">
                      <button
                        onClick={() => handleDeleteExpense(exp.id)}
                        className="p-1.5 text-slate-400 hover:text-rose-650 dark:hover:text-rose-400 rounded-lg transition-colors cursor-pointer opacity-80 group-hover:opacity-100"
